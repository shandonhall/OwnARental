...
function filterLinks(links: NavLinkDef[], role: string | undefined) {
  if (!role) return [];
  return links.filter(
...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const opsLinks = useMemo(
    () => filterLinks(OPS_NAV_LINKS, me.data?.role),
    [me.data?.role],
  );
  const demoLinks = useMemo(
    () => filterLinks(DEMO_NAV_LINKS, me.data?.role),
    [me.data?.role],
  );
  const allLinks = [...opsLinks, ...demoLinks];
  const canSearch = me.data
    ? hasPermission(me.data.role, 'SEARCH_READ')
    : false;
  const canNotify = me.data
    ? hasPermission(me.data.role, 'NOTIFICATIONS_READ')
    : false;
          ...
          <div className="mt-auto space-y-2 border-t border-slate-200 pt-4 dark:border-slate-700">
            {me.data ? (
              <div className="mb-3 text-xs text-brand-grey">
                ...
                <p className="mt-0.5">
                  {me.data.roleLabel ?? roleLabel(me.data.role)}
                </p>
              </div>
            ) : null}
            <button
          ...
          <header className="mb-6 flex flex-col gap-3 border-b border-slate-200 pb-4 dark:border-slate-700">
            <div className="flex items-center justify-between gap-3 md:hidden">
              ...
              <div className="flex items-center gap-2">
                {canNotify ? <NotificationCenter /> : null}
                <button
                  type="button"
                  onClick={signOut}
